...
Hirarki di atas memperlihatkan sebuah ruang-kerja yang terdiri dari dua
...
(Pengaturan yang umum lainnya yaitu menset `GOPATH=$HOME`.)
...
Untuk paket sendiri, sebisa mungkin pilihlah sebuah nama dasar _path_
...
Pada praktiknya, kita bisa memilih nama _path_ apapun, selama ia unik dengan
pustaka standar.
...
===  Program pertama
//{{{
Untuk mengkompilasi dan menjalankan sebuah program sederhana, pertama pilih
...
Ingatlah bahwa program tersebut bisa dijalankan dimanapun dalam sistem.
...
`hello` (atau, pada Windows, `hello.exe`).
...
Setelah memastikan paket `stringutil` dapat dibangun, ubahlah `hello.go`
...
Konvensi ini adalah cara termudah untuk membuat paket Go yang dapat digunakan
oleh orang lain.
Halaman
