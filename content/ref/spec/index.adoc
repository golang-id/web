...
Jika `M` ada dalam
http://127.0.0.1:6060/ref/spec#Method_sets[kumpulan method]
...
yaitu penerima (_receiver_) dari method.
...
Misalkan sebuah struct bertipe `T` dengan dua method, `Mv`, dengan penerima
bertipe `T`; dan `Mp` yang penerimanya bertipe `*T`.
...
menghasilkan sebuah fungsi yang sama dengan `Mv` namun dengan penerima
eksplisit sebagai argumen pertama; dengan penanda

----
func(tv T, a int) int
----

Fungsi tersebut bisa dipanggil secara normal dengan penerima eksplisit,
...
Untuk sebuah method dengan penerima nilai, kita dapat menurunkan sebuah fungsi
dengan penerima pointer yang eksplisit, sehingga
