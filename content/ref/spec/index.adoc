...
Literal rune merepresentasikan sebuah link:/ref/spec#Constants[konstan rune],
...
link:/ref/spec#Rune_literals[literal rune] (kecuali `\'` adalah ilegal dan `\"`
...
yang harus
link:/ref/spec#Import_declarations[diimpor].
...
link:/ref/spec#Order_of_evaluation[urutan evaluasi].
...
*  Sebuah elemen dengan sebuah kunci menggunakan kunci tersebut sebagai
   ...
   link:/ref/spec#Representability[dapat direpresentasikan]
...
link:/ref/spec#Address_operators[Mengambil alamat]
...
Ingatlah bahwa
link:/ref/spec#The_zero_value[nilai kosong] dari sebuah tipe
...
link:/ref/spec#Keywords[kata kunci]
...
Untuk sebuah
link:/ref/spec#Primary_expressions[ekspresi dasar]
`x` yang bukan sebuah
link:/ref/spec#Package_clause[nama paket],
_ekspresi selektor_
...
link:/ref/spec#Qualified_identifiers[pengidentifikasi
terbatas].
...
link:/ref/spec#Struct_types[field tertanam]
dari `T`.
...
.  Untuk nilai `x` bertipe `T` atau `*T` yang mana `T` bukanlah sebuah tipe
   ...
   Jika tidak ada
   link:/ref/spec#Uniqueness_of_identifiers[satu f]
...
.  Jika `x` adalah tipe pointer dan bernilai `nil` dan `x.f` menyatakan sebuah
   ...
   link:/ref/spec#Run_time_panics[panik _runtime_].
.  Jika `x` bertipe interface dan bernilai `nil`,
   link:/ref/spec#Calls[memanggil]
   atau
   link:/ref/spec#Method_values[mengevaluasi]
...
link:/ref/spec#Function_literals[literal fungsi]
atau
link:/ref/spec#Method_values[nilai method].
...
link:/ref/spec#Method_expressions[ekspresi method]
...
link:/ref/spec#Selectors[_selector_],
...
Seperti halnya dengan
link:/ref/spec#Calls[pemanggilan method],
...
Untuk `a` dengan
link:/ref/spec#Array_types[tipe Array]
A:
...
Untuk `a` sebagai
link:/ref/spec#Pointer_types[pointer]
ke tipe array:

*  `a[x]` adalah singkatan untuk `(*a)[x]`

Untuk `a` dengan
link:/ref/spec#Slice_types[tipe slice]
`S`:
...
Untuk `a` dari
link:/ref/spec#String_types[tipe string]:
...
Untuk `a` dari
link:/ref/spec#Map_types[tipe map]
`M`:

*  tipe `x` haruslah
   link:/ref/spec#Assignability[bisa disimpan]
   ke tipe key dari `M`
...
Kecuali untuk
link:/ref/spec#Constants[string tak bertipe],
...
link:/ref/spec#Address_operators[memiliki alamat]
...
link:/ref/spec#Type_identity[identik]
dengan tipe `T`.
Dalam kasus ini, `T` harus
link:/ref/spec#Method_sets[mengimplementasikan]
...
untuk tipe `T`.
Tidak ada
link:/ref/spec#Run_time_panics[panik _runtime]
...
yang dapat
link:/ref/spec#Assignability[disimpan]
...
link:/ref/spec#Order_of_evaluation[urutan yang biasa].
...
parameter dari `m`.
Jika `x` adalah
link:/ref/spec#Address_operators[alamat]
...
Jika `f` adalah
link:/ref/spec#Function_types[_variadic_]
...
link:/ref/spec#Comparison_operators[dibagian lain].
...
link:/ref/spec#Constant_expressions[ekspresi konstanta].
...
link:/ref/spec#Representability[direpresentasikan] oleh sebuah
nilai bertipe `uint`.
...
link:/ref/spec#Integer_overflow[integer overflow]
...
link:/ref/spec#Numeric_types[unsigned integer].
...
http://127.0.0.1:6060/ref/spec#Assignability[dapat disimpan]
...
*  Nilai pointer dapat dibandingkan.
   ...
   link:/ref/spec#Size_and_alignment_guarantees[ukuran nol]
...
*  Nilai interface dapat dibandingkan.
   ...
   http://127.0.0.1:6060/ref/spec#Type_identity[identik]
...
http://127.0.0.1:6060/ref/spec#Boolean_types[boolean]
